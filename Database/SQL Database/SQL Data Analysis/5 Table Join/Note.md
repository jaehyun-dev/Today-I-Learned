...
### 2. CROSS JOIN
CROSS JOIN은 한 테이블의 하나의 row에 다른 테이블의 모든 row들을 매칭하고, 그 다음 row에서도 또, 다른 테이블의 모든 row들을 매칭하는 것을 반복함으로써 두 테이블의 row들의 모든 조합을 보여주는 조인입니다.  
...
그냥 서로 별개인 두 테이블을 조인하는 것처럼 생각하시면 되는데요.  

![h](https://user-images.githubusercontent.com/71001479/221416154-2422266e-2c59-4232-9303-53d2ebb770ff.png)
...
### 4. FULL OUTER JOIN
FULL OUTER JOIN은 두 테이블의 LEFT OUTER JOIN 결과와 RIGHT OUTER JOIN 결과를 합치는 조인입니다.  
대신, 이때 두 결과에 모두 존재하는 row들(두 테이블에 공통으로 존재하던 row들)은 한번만 표현해주죠.

### 5. Non-Equi 조인
이때까지 우리는 조인 조건을 설정할 때 두 컬럼의 값이 같은지를 기준으로 했습니다. 즉, 조인 조건에 항상 등호(=)를 사용해왔죠.  
그런 조인들은 Equi 조인이라고 합니다. Equi는 Equality Condition의 줄임말로 동등 조건을 의미합니다. 이때까지 우리가 해온 조인은 모두 동등 조건을 판단하는 Equi 조인이었습니다.  
하지만 동등 조건이 아닌 다른 종류의 조건을 사용해서 조인을 할 수도 있는데요. 이런 조인을 Non-Equi 조인이라고 합니다.  

![m](https://user-images.githubusercontent.com/71001479/221416325-b0a1a8e4-6d6a-4d0f-8482-1b184293369f.png)
