...
질문 1  
해설:  
- 변수는 이름만 알면 어떤 값이 저장되어 있는지 알 필요가 없습니다.
- 함수는 그 구현 내용을 알지 못해도 파라미터만 잘 넣어주고 호출하면 사용할 수 있습니다.
- 클래스는 어떤 변수와 메소드가 정의되어 있는지만 알면 각각의 세부 구현 내용을 몰라도 사용할 수 있습니다.

질문 2  
해설: 너무 많은 코멘트는 오히려 코드를 읽는 개발자들을 헷갈리게 만들 수 있습니다. 코멘트는 꼭 필요할 때만 작성합니다.

질문 3  
해설:  
보기 1) (O) 추상화는 변수의 값이나, 메소드 내부의 내용을 모두 다 알지 않아도 그것을 사용할 수 있게 해줍니다.  
보기 2) (X) '클래스 내부의 속성을 외부로부터 숨길 수 있다'는 설명은 바로 다음에 배울 '캡슐화'에 대한 설명입니다.  
보기 3) (O) 클래스의 내용에 docstring을 달면 각 내용이 어떤 뜻인지를 더 잘 나타낼 수 있습니다.  
보기 4) (O) 프로그램에서 사용할 객체를 추상화 과정을 거쳐 설계하고 이를 클래스로 표현할 수 있습니다.  
