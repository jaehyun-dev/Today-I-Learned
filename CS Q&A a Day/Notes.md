...
- X, Y, Z라는 3가지 부분집합을 가진 릴레이션에서 {X -> Y}이고 {Y -> Z}일 때 {Y -> X}가 아니라면 암스트롱의 공리에 의해 {X -> Z}가 되는데 이 경우 X는 Z의 이행적 종속이라고 함
...
- 조회(SELECT) 속도를 향상시키지만, 데이터 모델의 유언성은 낮아짐
...
23.08.14
## Q. 디스크 캐시와 메모리(램) 캐시의 차이점에 대해 설명해주세요.

A.  
[23.03.01 질문 및 답변](https://github.com/jaehyun-dev/Today-I-Learned/blob/main/CS%20Q%26A%20a%20Day/Notes.md#q-%EB%94%94%EC%8A%A4%ED%81%AC-%EC%BA%90%EC%8B%9C%EC%99%80-%EB%A9%94%EB%AA%A8%EB%A6%AC%EB%9E%A8-%EC%BA%90%EC%8B%9C%EC%9D%98-%EC%B0%A8%EC%9D%B4%EC%A0%90%EC%97%90-%EB%8C%80%ED%95%B4-%EC%84%A4%EB%AA%85%ED%95%B4%EC%A3%BC%EC%84%B8%EC%9A%94%EC%9A%B4%EC%98%81%EC%B2%B4%EC%A0%9C)

23.08.15
## Q. 우선순위 큐의 동작방식에 대해서 설명해주세요.(자료구조)

A.  
우선순위 큐(Priority Queue)
- 들어간 순서에 상관없이 우선순위가 높은 데이터가 먼저 나오는 것
- 힙(Heap)을 통해 구현할 수 있음
- 힙에 데이터를 저장할 때는, 힙의 마지막에 데이터를 추가 후 heapify를 통해 우선순위가 높은 노드를 루트 노드로 보냄
...
23.08.26
## Q. RAID란 무엇인가요? RAID의 종류에 대해 설명해주세요.(운영체제)

A.  
RAID(Redundant Array of Inexpensive/Independent Disk)  
- 여러 개의 저장 장치(하드디스크 드라이브 등)를 묶어서 하나의 고용량/고성능 저장 장치처럼 사용하는 기술

구현 방식
1. 하드위어 방식
    - 별도의 물리적인 장치인 RAID 카드를 이용해서 RAID를 구현하는 방법
    - 서버와 같은 장치의 메인보드 PCLe 슬롯에 RAID 컨트롤러 카드를 별도로 장착하는 방법이 대표적
2. 펌웨어 방식
    - 비용이 비싼 하드웨어 방식의 RAID 카드 대신, 기능을 간소화한 RAID 칩을 메인보드에 탑재하여 펌웨어(드라이버)로 RAID를 구현하는 방법
    - 하드웨어 방식과 같이 운영체제에 진입하기 전 BIOS에서 RAID를 구성하며 운영체제와는 별개로 구성됨
3. 소프트웨어 방식
    - 소프트웨어를 통해 RAID를 구성하는 방식
    - 운영체제가 인식하고 있는 저장 장치들을 통해 RAID를 구성
...
3. RAID 2
    - 기록용 저장 공간과 복구용 저장 공간을 별도로 분리하여 구성하는 방식
4. RAID 3
    - Byte 단위로 스트라이핑을 하며 여러 체크 및 수정을 위해 패리티용 디스크 1개를 별도로 사용하는 방식
5. RAID 4
    - RAID 3과 동일한 방식이지만, 블록(Block) 단위로 스트라이핑을 하는 것이 차이점
6. RAID 5
    - RAID 4와 동일한 방식이지만, 에러 체크 및 수정을 위한 패리티 정보를 매번 다른 디스크에 저장하는 방식
7. RAID 6
    - RAID 5에서 에러 검출 및 수정을 위한 패리티 블록을 추가하여 이중화한 방식
...
23.08.28
## Q. Cross validation을 사용하는 이유와 장단점에 대해 설명해주세요.(머신러닝)

A.  
Cross Validation(교차 검증)
- 고정된 train set과 test set으로 평가를 하고, 반복적으로 모델을 튜닝하다보면 test set에만 과적합되어버리는 문제를 해결하기 위해 고안된 방법
- 데이터의 모든 부분을 사용하여 모델을 검증하고, test set을 하나로 고정하지 않음

장점
1. 모든 데이터 셋을 평가에 활용할 수 있음
    - 평가에 사용되는 데이터 편중을 막아 특정 평가 데이터 셋에 overfit 되는 것을 방지할 수 있음
...
23.08.29
## Q. 제3정규형의 조건에 대해서 설명해주세요.(데이터베이스)

A.  
제3정규형(3NF)
- 제2정규형이면서, 이행적 함수 종속성을 제거한 정규형
- 기본키에 속하지 않은 모든 속성이 기본키에 이행적 함수 종속이 아닌 정규형
...
23.08.30
## Q. 스누핑, 스푸핑, 스니핑의 개념에 대해 설명해주세요.(네트워크)
...
- 해커가 악용하고자 하는 호스트의 IP 주소를 바꾸어서 이를 통해 해킹을 하는 행위
- 네트워크 시스템에서 서로 신뢰관계에 있는 A, B 두 시스템 간에는 A시스템의 어카운트를 가지고 B시스템에 엑세스 할 수 있음
- 신뢰관계에 있는 두 시스템 사이에서 해커의 호스트를 마치 하나의 신뢰관계에 있는 호스트인 것처럼 속이는 것
...
23.09.01
## Q. 그래프를 구현함에 있어 인접행렬로 구현하는 방식과 인접리스트로 구현하는 방식의 장단점을 설명해주세요.(자료구조)

A.  
인접행렬  
- 장점
  - 구현이 쉬움
  - 노드 i와 노드 j의 연결 여부를 O(1) 시간복잡도로 확인 가능함
- 단점
  - 노드 i에 연결된 모든 노드에 방문하기 위해 O(V)의 시간복잡도 걸림(V는 노드의 개수)
  - 따라서, 간선의 개수에 비해 노드의 개수가 많은 그래프의 경우 비효율적임

인접리스트
- 장점
  - 모든 벡터들의 원소의 개수 합이 간선의 개수와 같으므로, 간선의 개수에 비례하는 메모리만 차지함
  - 노드 i에 연결된 모든 노드에 방문하기 위해 O(E)의 시간복잡도 걸림(E는 간선의 개수)
- 단점
  - 노드 i와 노드 j의 연결 여부를 확인하기 위해 O(V)의 시간복잡도 걸림
...
23.09.03
## Q. 제1정규형의 조건에 대해서 설명해주세요.(데이터베이스)

A.  
제1정규형(1NF)
- 릴레이션에 속하는 속성의 속성값이 모두 원자값(Atomic Value)만으로 구성되어야 함
- 즉, 속성값은 단일값이어야 하고 여러 개의 값을 가지면 안 됨

https://rebro.kr/160


23.09.04
## Q. URL과 URI의 차이점에 대해 설명해주세요.(네트워크)
...
23.09.06
## Q. LinkedList와 ArrayList의 차이점을 시간복잡도와 연관지어 설명해주세요.(자료구조)

A. 
LinkedList
- 각 노드는 값과 다음 노드의 주소로 구성됨
- 값을 읽어오기 위해서는 가장 앞에 있는 노드에서 시작하여 다음 노드 주소로 순차적으로 따라가야 하므로, O(N)의 시간복잡도가 걸림
- 특정 노드를 추가하기 위해서는 앞의 노드가 참조하는 주소와 현재 노드가 참조하는 주소를 변경하면 되므로, O(1)의 시간복잡도가 걸림
- 특정 노드를 삭제하기 위해서는 그 앞의 노드와 그 다음 노드를 연결하는 과정만 필요하므로, O(1)의 시간복잡도가 걸림

ArrayList
- 연속된 메모리에 값이 할당되어 있음
- 값을 읽어오기 위해서는 메모리의 시작 부분에서 인덱스만큼 더해준 주소를 확인하면 되므로, O(1)의 시간복잡도가 걸림
- 특정 값을 추가하기 위해서는 해당 인덱스부터 이후의 모든 값을 한 칸씩 뒤로 미뤄줘야 하므로 O(N)의 시간복잡도가 걸림
- 특정 값을 삭제하기 위해서는 해당 인덱스부터 이후의 모든 값을 한 칸씩 앞으로 당겨줘야 하므로 O(N)의 시간복잡도가 걸림

- 추가 및 삭제가 자주 일어나는 경우 LinkedList를, 탐색 및 읽기가 자주 일어나는 경우 ArrayList를 사용하는 게 유리

https://www.nextree.co.kr/p6506/


23.09.07
## Q. 머신러닝(Machine Learning)과 딥러닝(Deep Learning)의 차이에 대해 설명해주세요.(머신러닝)
...
딥러닝(Deep Learning)
- 알고리즘을 계층으로 구성하여 자체적으로 배우고 똑똑한 결정을 내릴 수 있는 '인공 신경망'을 만드는, 머신러닝의 하위 분야

차이점
- 딥러닝은 머신러닝의 하위 개념
- 머신러닝 모델은 새로운 데이터가 유입됨에 따라 특정 기능을 수행하는 데 점점 더 능숙해지만, 여전히 인간의 개입이 필요하며 AI 알고리즘이 부정확한 예측을 반환하면 엔지니어가 개입하여 조정해야 함
- 딥러닝 모델을 사용하면 알고리즘이 자체 신경망을 통해 예측의 정확성 여부를 스스로 판단할 수 있어 인간의 도움이 필요하지 않음
...
23.09.09
## Q. 브라우저 정책인 SOP와 CORS에 대해서 설명해주세요.(네트워크)
...
- 웹 애플리케이션은 자신의 출처와 동일한 리소스만 불러올 수 있으며, 다른 출처의 리소스를 불러오려면 그 출처에서 올바른 CORS 헤더를 포함한 응답을 반환해야 함
- 이는 시스템 수준에서 타 도메인 간 자원 호출을 승인하거나 차단하는 것을 결정하는 것
...
23.09.21
## Q. 데이터베이스 옵티마이저에 대해서 설명해주세요.(데이터베이스)

A.  
데이터베이스 옵티마이저(Optimizer)
- SQL을 가장 빠르고 효율적으로 수행할 최적의 처리경로를 생성해주는 DBMS 내부의 핵심 엔진
- 사용자가 구조화된 질의어(SQL)로 결과 집합을 요구하면, 이를 생성하는 데 필요한 처리 경로(실행 계획, Execution Plan)를 자동으로 생성해줌

최적화 과정
1. 사용자가 던진 쿼리 수행을 위해, 후보군이 될만한 실행계획을 찾음
2. 데이터 딕셔너리(Data Dictionary)에 미리 수집해 놓은 오브젝트 통계 및 시스템 통계정보를 이용해 각 실행계획의 예상 비용을 산정함
3. 각 실행계획을 비교하여 최저비용을 갖는 하나를 선택함

종류
- 규칙 기반 옵티마이저(RBO: Rule-Based Optimizer): 실행 속도가 빠른순으로 규칙(엑세스 경로 별 우선순위. 인덱스 구조, 연산, 조건절 형태가 순위를 결정짓는 주 요인)을 세워두고 우선순위가 높은 방법을 채택
- 비용 기반 옵티마이저(CBO: Cost-Based Optimizer): 예상 비용(쿼리를 수행하는 데 소용되는 일량 또는 시간)을 기반으로 최적화를 수행
...
A.  
[23.02.09 질문 및 답변](https://github.com/jaehyun-dev/Today-I-Learned/blob/main/CS%20Q%26A%20a%20Day/Notes.md#q-%EB%8D%B0%EC%9D%B4%ED%84%B0%EB%B2%A0%EC%9D%B4%EC%8A%A4%EC%9D%98-%EC%9D%B4%EC%83%81%ED%98%84%EC%83%81%EC%9D%98-%EC%A2%85%EB%A5%98%EC%99%80-%EA%B0%81%EA%B0%81%EC%9D%98-%EC%A0%95%EC%9D%98%EB%A5%BC-%EC%84%A4%EB%AA%85%ED%95%B4%EC%A3%BC%EC%84%B8%EC%9A%94%EB%8D%B0%EC%9D%B4%ED%84%B0%EB%B2%A0%EC%9D%B4%EC%8A%A4)


23.10.05
## Q. 라우터에서 발생하는 네트워크 지연요소들에 대해서 설명해주세요.(네트워크)

A.  
Delay: 하나의 데이터 패킷이 한 지점(출발지점)에서 다른 지점(도착 지점)으로 전송되기까지 소요되는 시간

1. 전송 지연(Transmission Delay)
   - 네트워크 장비의 데이터 전송 속도로 결정되며, Server-Client 간 거리와는 무관
   - 각각 10Mbps, 100Mbps의 속도를 가진 라인으로 10Mb를 전송하면, 첫 번째 라인은 1sec, 두 번째 라인은 0.1sec 소요, 지연 10배

2. 전파 지연(Propagation Delay)
   - 신호의 전송거리와 전송 매개체로 결정되지만, 네트워크 신호 전달 속도는 빛의 속도에 가깝기에 매우 작음
   - 전송거리(d)를 신호 전송 속도로 나누면 전파 지연 시간 얻을 수 있음

3. 노드 처리 지연(Node Processing Delay)
   - 라우터 자체 작업인 데이터 패킷 헤더의 처리, 비트 데이터 오류 등의 작업으로 소요되는 시간
   - 하드웨어서 패킷 헤더 또는 내부 데이터 일부 검사하는 과정에 소요되는 시간

4. 큐잉 지연(Queueing Delay)
   - 데이터 패킷을 라우터로 전송할 때 속도가 빠르면 라우터에서 제때 처리하지 못해 해당 패킷을 일시적으로 완충 구역인 큐에 머물러 대기 후 전송하는데, 이때 발생하는 지연 시간
...
23.10.16
## Q. 결정자에 대해서 설명해주세요.(데이터베이스)

A.  
결정자
- 주어진 릴레이션에서 다른 어트리뷰트를 고유하게 결정하는 하나 이상의 어트리뷰트
- 키 어트리뷰트 또는 복합 어트리뷰트일 수 있음
- A가 B를 결정하는 결정자라면 A -> B로 표기함

https://ybdeveloper.tistory.com/86


23.10.17
## Q. 서브넷팅에 대해서 설명해주세요.(네트워크)
...
23.10.19
## Q. 람다(lambda) 표현식에 대해 설명해주세요.(C/C++)

A.  
C++ 람다 표현식  
- 함수 이름이 없고 동작만 있는 함수
- 캡처, 매개변수 선언 부분, 함수 동작 부분, 함수 호출 시 인자 등으로 구성
...
A.  
[22.12.10 질문 및 답변](https://github.com/jaehyun-dev/Today-I-Learned/blob/main/CS%20Q%26A%20a%20Day/Notes.md#q-%EB%8D%B0%EC%9D%B4%ED%84%B0%EB%B2%A0%EC%9D%B4%EC%8A%A4-%ED%95%A8%EC%88%98-%EC%A2%85%EC%86%8D-%EC%A4%91-%EC%99%84%EC%A0%84-%ED%95%A8%EC%88%98-%EC%A2%85%EC%86%8D-%EB%B6%80%EB%B6%84-%ED%95%A8%EC%88%98-%EC%A2%85%EC%86%8D-%EC%9D%B4%ED%96%89%EC%A0%81-%ED%95%A8%EC%88%98-%EC%A2%85%EC%86%8D%EC%97%90-%EB%8C%80%ED%95%B4%EC%84%9C-%EC%84%A4%EB%AA%85%ED%95%B4%EC%A3%BC%EC%84%B8%EC%9A%94%EB%8D%B0%EC%9D%B4%ED%84%B0%EB%B2%A0%EC%9D%B4%EC%8A%A4)


23.10.21
## Q. IPv4와 IPv6의 차이점에 대해서 설명해주세요.(네트워크)
...
23.10.24
## Q. 클러스터링 인덱스란 무엇인지 설명해주세요.(데이터베이스)

A.  
클러스터형 인덱스(Clustered Index)
- 행 데이터를 해당 열로 정렬한 후에 루트 페이지를 만들어서 구성
- 데이터 위치를 바로 알기 때문에 그 데이터로 바로 접근할 수 있음
- 테이블당 1개씩만 허용됨
- 물리적으로 행을 재배열함
- PK 설정 시 그 컬럼은 자동으로 클러스터드 인덱스가 만들어짐
- 인덱스 자체의 리프 페이지가 곧 데이터이므로 테이블 자체가 인덱스임(따로 인덱스 페이지를 만들지 않음)
- 데이터 입력, 수정, 삭제 시 항상 정렬 상태를 유지함
- 비 클러스터형 인덱스보다 검색 속도는 더 빠르나 데이터의 입력, 수정, 삭제는 더 느림
...
23.10.28
## Q. 인덱스에 사용되는 자료구조를 2가지 이상 설명해주세요.(데이터베이스)

A.  
1. Hash Table
- Key-Value로 이루어진 데이터를 저장하는 데 특화된 자료구조
- 특정 컬럼의 값과 데이터의 위치를 Key-Value로 사용
- 내부에 버킷이라는 배열이 존재함
- 해시 함수를 통해 Key를 고유한 해시값으로 변환한 뒤, 이를 버킷 배열의 인덱스로 사용하며 해당 인덱스에 Value를 저장함
- Key값으로 Value가 저장되어 있는 위치(주소)를 바로 산출할 수 있으므로 O(1)의 평균 시간복잡도를 가짐
- 해시 함수를 제대로 정의하지 않으면 해시 충돌이 발생하고 검색 성능이 하락해 O(N)의 시간 복잡도를 가질 수 있음
- WHERE 조건의 등호(=) 연산에는 효율적이나 부등호 연산(>, <)에는 부적합함
- 내부 데이터들이 정렬되어 있지 않아 탐색이 비효율적임

2. B-Tree
- 자식 노드가 2개 이상인 트리
- 이진검색 트리처럼 각 Key의 왼쪽 자식은 항상 Key보다 작은 값, 오른쪽 자식은 큰 값을 가짐
- 특정 컬럼의 값(Key)에 해당하는 노드에 데이터의 위치(Value)를 저장함
- Key-Value 값들은 항상 Key를 기준으로 오름차순 정렬되어 있음
- 부등호 연산에 대해 해시 테이블보다 효율적 탐색이 가능함
- 균형 트리(Balanced Tree)로서, 최상위 루트 노드에서 리프 노드까지의 거리가 모두 동일하므로 O(logN)의 평균 시간 복잡도를 가짐
- 데이터 갱신(INSERT, UPDATE, DELETE)이 반복되면 트리의 균형이 깨지면서 성능이 악화됨
- 부등호를 이용한 검색 연산 성능은 좋지만, 순차 검색의 경우 중위 순회를 하므로 비효율적임

3. B+Tree
- B-Tree를 확장 및 개선한 자료 구조로서, 리프 노드에만 데이터의 위치(Value)를 관리함
- 중간 브랜치 노드에 Value가 없어서 B-Tree보다 메모리를 덜 차지하므로 노드의 메모리에 더 많은 Key를 저장할 수 있음
- 하나의 노드에 더 많은 Key를 저장하는 만큼 트리의 높이가 낮아짐
- 리프 노드들끼리는 LinkedList 구조로 서로를 참조하고 있음
- 부등호를 이용한 순차 검색 연산을 하는 경우 속도가 빠름
...
23.11.01
## Q. 그룹별로 묶인 집계함수에 대한 조건식을 설정하는 방법에 대해서 설명해주세요.(데이터베이스)

A.  
- GROUP BY 절을 사용하여 그룹별로 묶을 수 있음
- 그룹별 집계된 결과 중 원하는 조건의 결과만 필터링하기 위해서는 HAVING 절을 사용하여 필터 조건을 사용할 수 있음

https://gent.tistory.com/366


23.11.02
## Q. TCP의 자동재전송 기법 중 하나인 Go-Back-N ARQ에 대해서 설명해주세요.(네트워크)

A.  
Go-Back-N ARQ(Auto Repeat reQuest)
- 송신자는 확인응답을 기다리지 않고 여러 패킷을 전송할 수 있음
- 다만 파이프라인에서 확인 응답이 안 된 패킷의 최대 허용 수 N보다 크지 않아야 함
- 슬라이딩 윈도 프로토콜이라고 부르며 sequence number의 bit가 m bits일 때 2^m - 1 개까지 아웃스탠딩 프레임이 가능함
...
23.11.05
## Q. 프로시저의 장단점에 대해서 설명해주세요.(데이터베이스)

A.  
장점
- 프로시저만 수정이 필요할 경우 애플리케이션을 배포하지 않고 피로시저만 배포하면 됨
- 자연스럽게 프로시저에 인자를 추가하여 바인드 변수를 사용하게 되므로 SQL 하드 파싱을 걱정할 일이 없음
- SQL문을 캡슐화하여 여러 곳에서 재사용할 수 있음

단점
- 배포 절차가 따로 없으므로 이력(버전) 관리가 힘듦
- 길게 작성된 프로시저의 경우 로직 파악이 어려움
- 프로시저 내부에 연산이 포함될 경우 CPU 점유율이 높아지고 실행시간도 길어지므로 LCOK이 걸려있을 경우 병목이 될 확률이 높아짐

https://goldswan.tistory.com/51


23.11.06
## Q. TCP의 자동재전송 기법 중 하나인 Selective Repeat ARQ에 대해서 설명해주세요.(네트워크)

A.  
Selective Repeat ARQ(선택적 방법)
- 수신자에게 오류가 발생된 수신 패킷만을 다시 전송함
- 불필요한 재전송을 피할 수 있음
- GBN과 달리 버퍼 매니지먼트를 할 것이 많음
...
23.11.13
## Q. WHERE DNO ? (1, 2, 3)에서 ? 자리에 IN / ANY / ALL가 올 때의 차이에 대해서 설명해주세요.(데이터베이스)

A.  
IN
- 테이블의 row 중 DNO 컬럼의 값이 1, 2, 3 중 어느 하나라면 반환됨
- 즉, DNO = 1 OR DNO = 2 OR DNO = 3과 같은 결과

ANY
- 보통 비교 연산자와 함께 사용됨
- 예를 들어 DNO > ANY(1, 2, 3)인 경우 DNO가 1, 2, 3 중 어느 하나보다 큰 값을 가지는 로우를 반환함
- 즉, DNO > 1과 같은 결과

All
- 보통 비교 연산자와 함께 사용됨
- 예를 들어 DNO > ALL(1, 2, 3)인 경우 DNO가 1, 2, 3 모두에 비해 큰 값을 가지는 로우를 반환함
- 즉, DNO > 3과 같은 결과
...
A.  
트리거(Trigger)
- 데이터베이스가 갱신될 때마다 DBMS가 자동적으로 수행하도록 사용자가 정의하는 실행문
- 테이블 정의 시 표현할 수 없는 규칙들을 시행함
- 무결성 제약조건을 유지하기 위해 데이터베이스 갱신을 모니터링 및 진행함

장점
- 데이터베이스의 일관성을 유지하는 데 유용함
...
A.  
흐름제어
- 수신 측의 데이터 처리 속도보다 송신 측의 속도가 빠를 경우 문제가 생김
- 수신 측에서 제한된 저장 용량을 초과한 이후에 도착하는 패킷은 손실될 수 있고 불필요한 추가 패킷 전송이 발생하게 됨
...
23.11.27
## Q. TCP의 특징 중 하나인 혼잡제어의 역할에 대해서 설명해주세요.(네트워크)

A.  
혼잡제어
- 데이터의 양이 라우터가 처리할 수 있는 양을 초과하면 초과된 데이터는 라우터가 처리하지 못하고, 이때 송신 측에서는 라우터가 처리하지 못한 데이터를 손실 데이터로 간주하고 계속 재전송하여 네트워크를 혼잡하게 함
- 이런 상황은 송신 측의 전송 속도를 적절히 조절하여 예방할 수 있는데, 이것을 혼잡 제어라고 함
- 흐름 제어는 송수신 측 사이의 패킷 수를 제어하는 기능인 반면, 혼잡 제어는 네트워크 내의 패킷 수를 조절하여 네트워크의 오버플로우를 방지하는 기능
...
23.12.10
## Q. 도메인 제약조건에 대해서 설명해주세요.(데이터베이스)

A.  
[22.09.20 질문 및 답변](https://github.com/jaehyun-dev/Today-I-Learned/blob/main/CS%20Q%26A%20a%20Day/Notes.md#q-%EB%8F%84%EB%A9%94%EC%9D%B8-%EC%A0%9C%EC%95%BD%EC%A1%B0%EA%B1%B4%EC%97%90-%EB%8C%80%ED%95%B4%EC%84%9C-%EC%84%A4%EB%AA%85%ED%95%B4%EC%A3%BC%EC%84%B8%EC%9A%94)


22.12.11
## Q. 패킷교환 방식과 회선교환 방식의 차이점과 각 방식을 사용하는 예시를 들어주세요.(네트워크)

A.  
회선교환(Circuit Switching) 방식
- 회선 독점을 통한 통신방식
- 전송할 데이터가 있으면 전송을 위한 전용선을 할당하고 해당 선로로 모든 데이터를 전송함
- 통신 회선을 설정하여 데이터를 교환하는 방식
- 송신자의 모든 데이터는 동일한 경로로 전송됨
- 안정적인 통신이 가능함
- Point-To-Point 방식으로 연결됨
- 통신 중 중간경로에 문제가 발생할 경우 전체 연결이 끊어짐(새로운 경로를 통한 새로운 회선할당 필요)
- 대용량 + 고속 데이터 처리에 우수함
- 고정적인 대역폭을 사용함
- 연속적인 데이터 처리에 우수함
...
- 전용선의 개념이 없음
- 전송하려는 데이터를 패킷이라는 단위로 나눠 네트워크망으로 뿌려주게 됨
- 패킷에는 해당 데이터가 어떤 데이터의 몇 번째 데이터인지 정보와 최종 목적지에 대한 정보가 들어있음
- 해당 정보를 보고 라우터가 패킷을 거리, 망의 혼잡도(대역폭 사용률), 연결상태, 기타 설정 등 최적경로를 향해 전달하게 됨
- 패킷들은 라우터에 의해 서로 다른 경로로 전송될 수 있음
- 최종적으로 목적지에 도달한 뒤 패킷의 순서를 통해 다시 원래의 데이터로 합침
- 송신 패킷의 순서와 수신 패킷의 순서가 다를 수 있음
- 전송 속도 및 흐름 제어가 가능함
- 패킷 정보를 통해 에러 탐지가 가능함
- 회선의 이용률이 높음
- 에러 및 장애에 강함
- 경로 탐색과정에서 지연이 발생함
- 전송량 증가에 따라 지연율이 급격하게 상승함
- 패킷 헤더 추가로 인한 오버헤드 발생 가능함
- 예시) 인터넷 망
...
23.12.15
## Q. TCP의 헤더에는 무엇이 들어가나요? 아는대로 말씀해주세요.(네트워크)
...
A.  
[23.02.05 질문 및 답변](https://github.com/jaehyun-dev/Today-I-Learned/blob/main/CS%20Q%26A%20a%20Day/Notes.md#q-%EC%84%B8%EB%A7%88%ED%8F%AC%EC%96%B4semaphore%EC%99%80-%EB%AE%A4%ED%85%8D%EC%8A%A4mutex%EC%9D%98-%EC%B0%A8%EC%9D%B4%EC%A0%90%EC%97%90-%EB%8C%80%ED%95%B4-%EC%84%A4%EB%AA%85%ED%95%B4%EC%A3%BC%EC%84%B8%EC%9A%94%EC%9A%B4%EC%98%81%EC%B2%B4%EC%A0%9C)


23.12.17
## Q. array와 vector의 차이에 대해 설명해주세요.(C/C++)

A.  
1. 크기 조절 가능 여부
- 배열(Array): 정적 크기를 가지며, 선언할 때 고정된 크기는 변경할 수 없음
- 벡터(Vector): 동적 크기를 가지며, 런타임 중에 크기를 동적으로 조절할 수 있음
2. 메모리 할당 및 해제
- 배열: 컴파일 시간에 정적 메모리 할당이 이루어지며, 프로그램이 종료될 때까지 메모리가 유지됨
- 벡터: 동적으로 메모리를 할당하며, 벡터의 수명이 끝나면 자동으로 메모리가 해제됨
3. 편의성 및 기능
- 배열: 기본적이고 간단한 데이터 구조로, 특별한 기능이나 메서드가 없음
- 벡터: <vector> 헤더에서 제공하는 다양한 멤버 함수를 사용하여 요소의 삽입, 삭제, 검색 등 다양한 작업을 수행할 수 있음
4. 반복자 및 내장
- 배열: 직접적인 반복자(iterator)를 제공하지 않으며 인덱스를 사용하여 반복이 이루어짐
- 벡터: STL(Standard Template Library)에서 제공하는 반복자를 통해 벡터의 요소에 접근하고 조작할 수 있음
5. 복사 및 이동
- 배열: 복사와 이동이 일반적으로 메모리 블록 복사를 통해 이루어짐
- 벡터: 이동 시에는 메모리 소유궝니 전달되어 더 효율적인 작업이 이루어짐

[출처: ChatGPT]


23.12.18
## Q. 키 제약조건에 대해서 설명해주세요.(데이터베이스)
...
23.12.19
## Q. TCP의 연결수립 과정에 대해서 설명해주세요.(네트워크)

A.  
3 Way-Handshake
- TCP에서 통신을 하는 장치 간 서로 연결이 잘 되어있는지 확인하는 과정 및 방식
- 송수신자 사이에 연결을 확인하는 과정
1. 클라이언트가 연결 요청 메시지(SYN)을 전송
2. 서버가 요청을 수락하고 클라이언트에게도 연결 요청 메시지를 전송
3. 클라이언트가 서버에 응답을 전송
...
A.  
- static_cast
    - 컴파일(정적) 타임에 형변환이 가능한지 검사함
    - 사용자 정의 클래스가 아닌 C++ 내부에 이미 정의된 기본 자료형간의 형변환 하고싶을 때 사용 가능
- dynamic_cast
    - 런타임 중에 안정성 검사를 진행함
    - 상속관계에서의 형변환을 보다 안전하게 처리할 수 있음
    - 자식 클래스의 참조/포인터 형식에서 부모 클래스의 참조/포인터 형식으로 형변환을 허용함
- reinterpret_cast
    - 포인터/참조와 관련된 형변환만 지원함
    - dynamic_cast보다 훨씬 강력하며 C 스타일의 수준으로 형변환이 가능함
- const_cast
    - const 성질을 제거하고 싶을 때 사용함
...
23.12.26
## Q. 외래키와 참조 무결성 제약 조건에 대해서 설명해주세요.(데이터베이스)

A.  
외래키와 참조 무결성 제약조건(referential integrity constraint)
- 참조 무결성 제약조건은 두 릴레이션의 연관된 튜플들 사이의 일관성을 유지하는 데 사용됨
- 관계 데이터베이스가 릴레이션들로만 이루어지고, 릴레이션 사이의 관계들이 다른 릴레이션의 기본키를 참조하는 것을 기반으로 하여 묵시적으로 표현되기 때문에 외래키의 개념이 중요
- 릴레이션 R2의 외래키가 릴레이션 R1의 기본키를 참조할 때 참조 무결성 제약조건은 아래의 두 조건 중 하나가 성립되면 만족됨
    - 외래키의 값은 R1의 어떤 튜플의 기본키 값과 같다(기본키 목록 중 하나이어야 함)
    - 만약 그렇지 않다면 NULL값을 가짐
    - 단, 외래키가 자신을 포함하고 있는 릴레이션의 기본키를 구성하고 있지 않음
...
23.12.29
## Q. C++ 프로그램의 빌드과정에 대해 설명해주세요.(C/C++)

A.  
1. 전처리
    - C++ 소스코드에서 #include나 #define, #ifdef 등을 컴파일러가 이해할 수 있도록 코드로 변환시켜줌
    - #include 같은 경우 헤더 파일(.h)의 내용을 삽입해주고 #define은 해당 값으로 치환해주며, #ifdef 같은 경우는 지정한 define값이 있거나 없을 때 작성한 소스코드로 치환함

2. 컴파일
    - 각 운영체제에서 사용할 수 있는 컴파일러로 전처리가 완료된 C++ 코드를 컴파일함
    - 컴파일 과정은 front 어휘 분석 -> 구문 분석 -> 의미 분석 -> 코드 생성 순서로 진행됨
    - 컴파일이 완료된 C++ 소스코드는 어셈블리어로 변환됨

3. 어셈블
    - 어셈블리어로 변환된 소스코드를 컴퓨터가 이해할 수 있도록 어셈블러를 통해 기계어로 번역해줌.
    - 0, 1로 이루어져 있으며 오브젝트 코드라고 불림
    - 오브젝트 코드는 링킹 과정에서 필요한 정보를 가지고 있음
...
24.01.10
## Q. DNS에 대해서 설명해주세요.(네트워크)

A. 
DNS(Domain Name System, 도메인 네임 시스템)
- 웹사이트에 접속할 때 입력한 도메인 이름을 실제 네트워크 상에서 사용하는 IP주소로 바꾸고 접속하는 과정 및 시스템
- 이러한 시스템은 전세계적으로 약속된 규칙을 공유함
- 상위 기관에서 인증된 기관에게 도메인을 생성하거나 IP주소를 변경할 수 있는 권한을 부여함
- 상위 기관, 하위 기관과 같이 '계층 구조'를 가지는 분산 데이터베이스 구조를 가짐
...
버퍼
- 임시 메모리 공간
- 입출력을 바로바로 전달하지 않고 버퍼에 담아둘 수 있음
- 문자를 한 번에 전달하므로 전송시간이 적게 걸림
- 문자를 잘못 입력했을 경우 수정할 수 있음
- 시스템 콜을 적게 호출해 자원을 아낄 수 있음

std::endl
- 입력되면 버퍼가 자동으로 비워지는 개행 문자
- 버퍼를 비워야 하기에 속도가 느림
...
24.01.15
## Q. virtual에 대해 설명하고 가상함수를 사용해야 하는 이유에 대해 설명해주세요.(C/C++)

A.  
가상함수
- 부모 클래스에서 상속받을 클래스에서 재정의할 것으로 기대하고 정의해놓은 함수
- virtual이라는 예약어를 함수 앞에 붙여서 생성할 수 있음
- 생성된 가상함수는 파생 클래스에서 재정의하면 이전에 정의되었던 내용들도 모두 새롭게 정의된 내용들로 교체됨

사용해야 하는 이유
- 컴파일러는 함수를 호출하는 코드를 컴파일 타임에 고정된 메모리 주소로 변환시키는 정적바인딩을 함
- 일반함수의 경우 모두 정적바인딩을 하므로, 오버로딩하게 되면 문제가 생길 수 있음
...
POST
- 리소스를 생성/변경하기 위해 설계되었기 때문에 전송해야될 데이터를 HTTP 메시지의 Body에 담아서 전송함
- HTTP 메시지의 Body는 길이의 제한없이 데이터를 전송할 수 있으므로 대용량 데이터를 전송할 수 있음
- 내용이 눈에 보이지 않아 보안적인 면에서 GET보다는 안전하지만 개발자 도구에서 요청 내용을 확인할 수 있음
...
차이점
- 사용목적: GET은 서버의 리소스에서 데이터를 요청할 때, POST는 서버의 리소스를 새로 생성하거나 업데이트할 때 사용함. DB로 따지면 GET은 SELECT에, POST는 CREATE에 가까움
- 요청의 body 유무: GET은 URL 파라미터에 요청하는 데이터를 담아 보내기 때문에 HTTP 메시지에 body가 없는 반면 POST는 body에 데이터를 담아 보내기 때문에 HTTP 메시지에 body가 존재함
- 멱등성(idempotent): GET은 멱등이며 POST는 멱등이 아님
...
24.01.19
## Q. 기본생성자(default constructor)와 복사생성자(copy constructor)에 대해 설명해주세요.(C/C++)

A.  
기본생성자(디폴트 생성자)
- 인수를 취하지 않는 생성자
- 멤버에 특정값을 대입하는 대신 무난한 값으로 초기화하는 역할을 함
- 디폴트 생성자를 호출할 때에는 객체 선언문에 빈 괄호를 붙이지 않으며 아예 괄호가 없어야 함
- 또는 명시적으로 생성자를 호출하면서 괄호를 붙여야 함
```C++
// 맞음
Human momo;
Human momo = Human();
...
Human momo();
```
- 생성자를 정의하지 않으면 컴파일러가 디폴트 생성자를 자동으로 정의하고, 쓰레기값이 들어가 초기화됨

복사생성자
- 이미 선언된 객체로부터 같은 타입의 객체를 복사할 때 사용하는 생성자
- 디폴트 복사 생성자는 멤버값을 일대일로 대입할 뿐이어서 두 객체의 포인터가 같은 번지를 가리키는 반면, 직접 복사 생성자를 정의하면 사본에 별도의 메모리를 할당할 수 있음

https://andjjip.tistory.com/255


24.01.20
## Q. 최근 HTTP 3.0을 많이 사용하고 있는데 특징에 대해서 설명해주세요.(네트워크)

A.  
HTTP/3.0은 Hypertext Transfer Protocol의 세 번째 주요 버전으로, 전송 계층에서 사용되는 프로토콜인 QUIC(Quick UDP Internet Connections) 위에서 동작합니다. HTTP/3.0은 이전 버전인 HTTP/1.1 및 HTTP/2.0과 비교하여 몇 가지 주요 특징을 가지고 있습니다.

1. 전송 프로토콜:
HTTP/3.0은 기존의 TCP 대신에 QUIC 프로토콜을 사용합니다. QUIC은 UDP를 기반으로 하며, 여러 가지 이점을 제공합니다. 이를 통해 연결 설정과 해제 과정이 간소화되고, 빠른 연결 설정 및 다양한 기능이 제공됩니다.

2. 다중화:
HTTP/3.0은 이전 버전과 마찬가지로 여러 개의 요청과 응답을 동시에 처리할 수 있는 다중화 기능을 제공합니다. 이는 웹 페이지 로딩 속도를 향상시키고 효율적인 리소스 관리를 가능케 합니다.

3. 헤더 압축:
HTTP/3.0에서는 헤더 압축 기술로 HPACK 대신 QPACK을 사용합니다. QPACK은 헤더 압축을 효과적으로 처리하면서도 중요한 기능인 동적 테이블 크기 조절을 가능하게 합니다.

4. 초기화 속도:
QUIC은 연결 설정과 초기화 속도를 향상시키기 위해 설계되었습니다. 기존의 TCP 연결에서 발생하는 핸드셰이크 지연을 줄이고, 빠른 연결 설정을 지원하여 웹 페이지의 로딩 시간을 단축합니다.

5. 신뢰성 및 견고성:
QUIC은 연결이 끊어진 경우에도 신속하게 복구될 수 있는 기능을 제공하여 네트워크의 불안정성에 대응합니다. 이로써 끊김이나 패킷 손실이 발생해도 사용자 경험이 향상됩니다.

6. 보안 강화:
QUIC은 기본적으로 암호화를 사용하며, 이는 연결의 기밀성과 무결성을 강화합니다. 또한, 이전 버전에서 발생할 수 있던 일부 보안 취약성을 개선하였습니다.

HTTP/3.0은 앞으로의 웹 통신에서 높은 효율성과 빠른 성능을 제공하기 위한 노력의 결과물 중 하나로, 점차적으로 많은 웹 서버와 브라우저에서 지원되고 사용되고 있습니다.

[출처] ChatGPT 답변


24.01.21
## Q. 생성자(constructor)와 소멸자(destructor)에 대해 설명해주세요.(C/C++)

A.  
클래스를 가지고 객체를 생성할 때(클래스의 인스턴스화) 자동으로 생성자를 호출하고 클래스가 소멸될 때 소멸자가 자동으로 호출됨

생성자(constructor)
- 객체를 생성하는 함수
- 일반 함수들과 다르게 return값을 적어주지 않음
- 클래스명과 같은 이름으로 만들어야 함
- 생성자를 따로 정의하지 않으면 자동으로 기본생성자를 컴파일러가 만들어줌
- 생성자를 하나라도 오버로딩하면 기본생성자는 개발자가 직접 넣어주어야 함

소멸자(destructor)
- 객체가 소멸할 때 호출되는 함수
- return값이 없음
- 소멸자를 따로 정의하지 않으면 자동으로 기본소멸자를 컴파일러가 만들어줌
- 클래스 안의 동적으로 할당된 멤버변수가 있다면 소멸자 안에 메모리를 풀어주는 코드를 넣어주어야 함
...
24.01.22
## Q. HTTP의 상태코드에 대해서 설명해주시고 자주 사용하는 상태코드에는 무엇이 있나요?(네트워크)

A.  
HTTP 상태코드(Status Code)
- 웹서비스에서 요청에 따라 반환되는 정보의 상태를 나타내는 코드
- 3자리 숫자로 만들어져 있으며, 첫 번째 자리는 1에서 5까지 있음
- 첫 번째 자리가 4와 5인 경우는 정상적인 상황이 아니기 때문에 사이트 관리자가 즉시 알아야 하는 정보
...
5. 5xx(서버 오류): 서버가 명백히 유효한 요청에 대한 충족을 실패함

자주 사용하는 상태코드
- 200 OK: 요청이 성공적으로 되었고, 정보는 요청에 따른 응답으로 반환됨
...
[23.02.28 질문 및 답변 참고](https://github.com/jaehyun-dev/Today-I-Learned/blob/main/CS%20Q%26A%20a%20Day/Notes.md#q-http-%EC%83%81%ED%83%9C%EC%BD%94%EB%93%9Cstatus-code%EC%9D%98-%EC%9D%98%EB%AF%B8%EB%A5%BC-%EC%84%A4%EB%AA%85%ED%95%B4%EC%A3%BC%EC%84%B8%EC%9A%94%EB%84%A4%ED%8A%B8%EC%9B%8C%ED%81%AC)


24.01.23
## Q. 선언과 정의의 차이에 대해 설명해주세요.(C/C++)

A.  
선언(declaration)
- 컴파일러가 참조할 식별자(identifier)와 이름을 알림
- 메모리 영역 상에 올리지 않기 때문에 중복되어도 문제가 되지 않으며 두 번 이상 할 수 있음

정의(definition)
- 식별자와 이름으로부터 코드를 생성하여, 함수가 호출되거나 변수를 사용할 때 생성된 코드를 참조함
- 고유 사양으로 프로그램에는 정의가 하나만 있어야 함
- 같은 식별자와 이름의 정의가 두 개 이상이라면 컴파일 에러가 발생함
- 선언만 하고 정의를 하지 않은 경우 링커 단계에서 참조할 코드가 없기 때문에 링커 에러가 발생함
    - 다만 예외 사항으로 아래의 경우 정의가 필요없음
        1. 함수가 선언만 있고 정의가 없지만 함수를 호출하거나 함수의 주소를 참조하는 일이 없음
        2. 정의를 알 필요가 없는 방식으로만 사용됨(전방 선언 등)
...
C++언어
- 객체지향 프로그래밍(Object Oriented Programming)(캡슐화, 상속성, 다형성)
- 코드의 재활용성이 높음
- 구조화 되어있어 오류 수정하기 쉬움
- 이식성이 좋고 유연함
- 객체의 역할과 기능을 이해해야 하므로 복잡함
- 절차지향언어보다 속도가 느림
- 메모리가 안전하지 않음
...
24.01.28
## Q. HTTP의 요청 메서드에 대해서 설명해주세요.(네트워크)

A.  
HEAD
- 메시지 헤더(문서 정보) 취득
- GET과 유사한 방식이지만, 실제 문서를 요청하는 것이 아니라 문서 정보를 요청하는 것
- HTTP 응답 메시지에 본문(Body) 없이 HTTP 헤더 정보만을 보냄

GET
- URI 형식으로 웹 서버 측 리소스(데이터)를 요청
- 파라미터를 넘겨서 해당하는 본문형식을 받음
...
PUT
- 내용 갱신 위주(파일 전송 가능)
- POST처럼 정보를 서버로 제출하나 보통 갱신 위주

DELETE
- 파일 삭제
- 웹 리소스를 제거할 때 사용함
- 서버에서 클라이언트의 요청을 무시 가능하기 때문에 실제로 삭제되지 않았지만 클라이언트는 파일이 삭제되었다고 생각할 수 있음

OPTIONS
- 웹 서버 측 제공 메서드에 대한 질의
- 응답 메시지에 HTTP 헤더 항목 중 'Allow: GET, POST, HEAD'처럼 보내게 됨

Trace
- 요청 리소스가 수신되는 경로를 보여줌
- 자기 앞으로 요청 메시지를 반환(루프백) 시험

Connect
- 프록시 서버와 같은 중간 서버 경유
- 요청한 리소스에 대해 양방향 연결을 시작하는 메서드
- SSL(HTTPS)를 사용하는 웹사이트에 접속하는 데 사용할 수 있음
- 클라이언트는 원하는 목적지와 TCP 연결을 HTTP 프록시 서버에 요청하고, 서버는 클라이언트 대신 연결의 생성을 진행함
...
- HTML, Plain text, JSON, XML 등 다양한 형태의 정보를 전송하는 애플리케이션 레이어 프로토콜
- 웹 브라우저와 웹 서버 간의 커뮤니케이션을 위해 디자인됨
- 최근에는 모바일 애플리케이션 및 IoT 등의 커뮤니케이션 목적으로도 사용됨
- 클라이언트가 요청을 생성하기 위한 연결을 연 다음 응답을 받을 때까지 대기하는 전통적인 클라이언트-서버 모델을 따름
...
24.02.04
## Q. OSI 7계층과 TCP/IP 4계층의 차이점에 대해서 설명해주세요.(네트워크)

A.  
OSI(Open Systems Interconnection) 7계층
- 7개 계층으로 구성되며, 물리, 데이터 링크, 네트워크, 전송, 세션, 표현, 응용으로 나뉨
- 상대적으로 더 복잡하고 추상적인 구조를 가지고 있음
- 각 계층이 서로 다른 프로토콜 집합을 정의함
- 주로 교육 및 이론적인 목적으로 사용됨
- 국제 표준화 기구(ISO)에서 정의한 표준

TCP/IP(Transmission Control Protocol/Internet Protocol) 4계층
- 4개 계층으로 구성되며, 네트워크 액세스, 인터넷, 전송, 응용으로 나뉨
- 상대적으로 더 간결하고 인터넷의 실제 운용 환경에 더 적합한 간소화된 구조를 제공함
- 계층 간의 프로토콜 상호 작용이 더 강력하며, 종종 여러 계층에서 하나의 프로토콜을 사용함
- 현재 인터넷과 네트워크 기술에서 실제로 적용되고 있는 모델로, 보다 현업에서 더 널리 사용됨
- 초기에는 군사 및 학계에서 개발되었으며, 현재는 인터넷 표준으로 자리잡음

공통점
- 네트워크 프로토콜 스택의 모델로, 네트워크 통신을 구성하는 계층을 정의함
...
24.02.06
## Q. 전송계층의 기능 중 멀티플렉싱과 디멀티플렉싱 기술에 대해 설명해주세요.(네트워크)

A.  
[22.09.13 질문 및 답변](https://github.com/jaehyun-dev/Today-I-Learned/blob/main/CS%20Q%26A%20a%20Day/Notes.md#q-%EC%A0%84%EC%86%A1%EA%B3%84%EC%B8%B5%EC%9D%98-%EA%B8%B0%EB%8A%A5-%EC%A4%91-%EB%A9%80%ED%8B%B0%ED%94%8C%EB%A0%89%EC%8B%B1%EA%B3%BC-%EB%94%94%EB%A9%80%ED%8B%B0%ED%94%8C%EB%A0%89%EC%8B%B1-%EA%B8%B0%EC%88%A0%EC%97%90-%EB%8C%80%ED%95%B4-%EC%84%A4%EB%AA%85%ED%95%B4%EC%A3%BC%EC%84%B8%EC%9A%94)
