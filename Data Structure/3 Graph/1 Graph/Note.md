...
### 실습 설명
저번 영상에서는 인접 행렬에 대해서 배웠는데요. 이번 과제에서는 한번 직접 구현해 볼게요. 노드들을 만들지는 않을 거고 엣지들만 저장해 보겠습니다.

![image](https://bakey-api.codeit.kr/files/2418/qKNZq9?name=1.png)  

위에 있는 무방향 그래프를 구현해 볼게요. 각 노드 위나 옆에 있는 정수가 노드의 고유 숫자입니다. 이 고유 숫자를 행렬의 인덱스로 사용하면 되는 거죠.

지금 템플릿에는 0으로 채워진 6 x 6 크기의 행렬이 있는데요. 행렬을 수정해서 위 그래프의 엣지 정보를 저장해 보세요.
...
무방향 그래프 엣지는 쌍방의 관계입니다. 하나의 엣지에 대해서 두 개의 정보를 저장해야 됩니다.

엣지 (영훈, 현승)를 저장해 볼게요. 그럼 무방향 그래프에서는 (영훈, 현승), (현승, 영훈) 엣지를 모두 저장해야 된다는 거죠. 그러니까 영훈에 해당하는 인덱스가 0, 현승에 해당하는 인덱스가 1이잖아요? 그럼 아래 코드처럼 0과 1을 인덱스로 사용해서 해당 요소에 있는 데이터가 인접해 있다는 표시, 1을 저장하면 되는 거죠.
```python
adjacency_matrix[0][1] = 1
adjacency_matrix[1][0] = 1
```
그래프 안에 있는 모든 엣지에 똑같은 작업을 반복하면 됩니다. 이렇게 해주면 되겠죠?
```python
# 엣지 (영훈, 현승) 저장
adjacency_matrix[0][1] = 1
adjacency_matrix[1][0] = 1
...
### 실습 결과
모든 엣지들이 인접 행렬에 제대로 저장된 걸 확인할 수 있습니다. (실제로는 줄바꿈 없이 출력됩니다.)
```
[
[0, 1, 1, 0, 0, 0],
[1, 0, 0, 1, 0, 1],
...
[main1_09.py](https://github.com/jaehyun-dev/Today-I-Learned/blob/dd8f7e6b641decc3a0d0403ee33a7aeec450dbe6/Data%20Structure/3%20Graph/1%20Graph/main1_09.py) 참고
...
노드들의 인접 데이터를 리스트에 저장하는 방법  

### 인접 리스트

그래프 노드를 클래스로 구현할 때, self.adjacent_node = [] 리스트를 만들어줌  
이 리스트에 인접하는(연결된) 노드를 저장하면 됨  

무방향 그래프에서 A와 B가 연결되어 있으면 A의 인접 리스트에도 B가, B의 인접 리스트에도 A가 저장  
그러나 방향 그래프에서는, A를 떠나서 B로 들어가는 엣지가 있으면 A의 인접 리스트에 B를 저장하고 반대는 저장하지 않음  
가중치 그래프에서는, 튜플을 이용해서 (B, 4)와 같이 인접한 노드와 가중치를 함께 저장해줌  
